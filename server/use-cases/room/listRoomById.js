//@ts-check

/**
 *
 * @param {{ roomDB: import("../../interface").returnMakeRoomDB, roomIdIsValid: Function }} parameter
 */
export default function makeListRoomById({ roomDB, roomIdIsValid }) {
  /**
   * @param {{ room_id:string }} room_id
   */
  return async function listRoomById({ room_id }) {
    if (!roomIdIsValid(room_id)) {
      throw new Error('This is not a valid room id');
    }

    const result = await roomDB.findRoomById({ id: room_id });
    return result;
  };
}
