 ...
 */
export default function makeListPlayer({ playerDB, roomIdIsValid }) {
  /**
   * @param {{ room_id: string, socket_id: string }} parameters
   */
  return async function listPlayer({ room_id, socket_id }) {
    if (!roomIdIsValid(room_id)) {
      throw new Error('This is not a valid room id');
    }

    const result = await playerDB.findOnePlayer({ room_id, socket_id });
    return result;
  };
}
